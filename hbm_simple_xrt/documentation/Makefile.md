...
| `make test_systolic_array`  | Compile + run 32×32 BF16 systolic array tests (19 tests)          |
| `make test_mxu`             | Compile + run 32×32 BF16 MXU tests (19 tests)                     |
| `make test_matmul`          | Compile + run 32×32 BF16 matmul_top HBM integration tests (9 tests)|
...
```
SRC_BF16     = bf16_add.sv, bf16_mul.sv
SRC_PE       = pe.sv + SRC_BF16
...
- **N is baked in:** All modules default to N=32. Override with environment variables (`SYSTOLIC_N`, `MXU_N`, `MATMUL_N`, `KRNL_N`) if needed for debug.
