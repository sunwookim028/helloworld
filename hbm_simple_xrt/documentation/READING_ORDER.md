# Reading Order
You have an RTL/digital design background but have never seen this codebase. The project has two subsystems: a proven HBM DMA kernel (bandwidth reference + burst AXI patterns) and a 32×32 BF16 systolic array matmul kernel. Read them in the order below.
...
A standalone DMA kernel used to validate HBM burst bandwidth and fix the >4KB AXI hang. Its burst read/write pattern is integrated into `krnl_matmul` for high-throughput HBM access. Read bottom-up.
...
32×32 BF16 matrix multiply: `OUT = X × W^T`. Read bottom-up through the datapath, then the controller.

### 6. `src/bf16_mul.sv` → [bf16_mul.md](bf16_mul.md)
Combinational BF16 multiplier — pure `always @(*)`. Skim special-case handling (NaN, Inf, zero), then the normal mantissa multiply. Key difference from FP32: 7-bit mantissa, 18-bit intermediate product. Takeaway: `result = a × b` in one combinational pass, zero latency.

### 7. `src/bf16_add.sv` → [bf16_add.md](bf16_add.md)
Combinational BF16 adder. Same idea. Both arithmetic blocks are **zero-latency**.

### 8. `src/pe.sv` → [pe.md](pe.md)
A PE instantiates one `bf16_mul` and one `bf16_add`: `psum_out = (input × weight) + psum_in`. Focus on:
...
Bridges the 512-bit HBM interface to MXU's 16-bit (BF16) element interface. Read after completing Parts 1 and 2.

### 11. `src/matmul_top.sv` → [matmul_top.md](matmul_top.md)
Focus on:
1. **Localparam arithmetic**: `ELEMS_PER_WORD=32`, `WORDS_PER_MATRIX=32` for N=32 BF16
...
4. **Store pack loop**: 16-bit BF16 `out_bram` elements packed into one 512-bit `mem_wr_data` word
...
- Address routing: `w_offs = mt_mem_addr - addr_w_word; if w_offs < 32 → gmem0 else → gmem1`
- Burst mode: one 32-beat burst per matrix (ARLEN=31)
...
`memory_driver()` models the 16-bit BF16 BRAM interface. `run_matmul()` just loads matrices, pulses start, waits for done. Reference computed by `bf16_ref()`.
...
Read last. Complete data flow diagrams, systolic timing, verification strategy, design pitfalls, and origin/lineage table.

---

## Summary
**Steps 1–5**: HBM DMA engine — burst AXI4 patterns, FWFT FIFO idiom.  
**Steps 6–10**: Systolic array — BF16 arithmetic → PE → 32×32 grid → MXU controller.  
**Step 11**: HBM bridge — 512-bit packing (32 BF16 elements per word), handshake memory interface.  
...
- **Compute:** the systolic array computes OUT = X × W^T using weight-stationary dataflow with staggered activation feeding; each PE performs one BF16 MAC per cycle.
- **Integration:** matmul_top unpacks 512-bit HBM words into per-element BRAMs (32 BF16 elems/word), runs the MXU, repacks results — bridge between memory bandwidth and compute. krnl_matmul wraps this with Vitis AXI4 master ports.
