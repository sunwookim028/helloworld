...
void krnl_vadd_desc(
    ...
    ap_uint<512> *data_mem,          // Unified data memory (read/write)
...
#pragma HLS INTERFACE m_axi port=data_mem offset=slave bundle=gmem_data
#pragma HLS INTERFACE s_axilite port=desc_mem bundle=control
#pragma HLS INTERFACE s_axilite port=data_mem bundle=control
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    //
    // NOTE: Uses single unified data_mem port for both reads and writes.
    // This matches real DMA engine architecture (single AXI master).
    // Addresses in descriptors are relative to data_mem base address.
    // =========================================================================
    ...
    while (desc_count < max_descriptors) {
        ...
        // Uses unified data_mem for both reads and writes
        data_transfer_loop:
        for (uint32_t i = 0; i < num_words; i++) {
            #pragma HLS PIPELINE II=1
            #pragma HLS LOOP_TRIPCOUNT min=16 max=65536 avg=1024
            
            // Read from source through unified data memory
            ap_uint<512> data = data_mem[src_word_idx + i];
            
            // Write to destination through unified data memory
            data_mem[dst_word_idx + i] = data;
        }
