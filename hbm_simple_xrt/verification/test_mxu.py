"""
Cocotb test suite for the parameterized MXU (systolic array wrapper, BF16).
...
Memory stores 16-bit BF16 values at element-granularity addresses.
"""
...
# =============================================================================
# Memory model — 16-bit BF16 elements at element-granularity addresses
# =============================================================================
async def memory_driver(dut, mem):
    """
    Simple memory model:
      - On read_en: latch address, output mem[addr] next cycle
      - On write_en: write mem[addr] = data (16-bit BF16)
    """
    last_addr = 0
    while True:
...
def load_matrices(mem, W, X):
    """Load W and X (NxN float32) into the memory map as BF16 elements."""
    for i in range(N):
        for j in range(N):
...
def read_output(mem):
    """Read NxN output matrix from memory (BF16 → float32)."""
...
async def run_matmul(dut, mem, W, X):
    ...
    for _ in range(TIMEOUT_CYCLES):
        ...
        raise cocotb.result.TestFailure(f"Timeout after {TIMEOUT_CYCLES} cycles")

    await RisingEdge(dut.clk)
    return read_output(mem)


def assert_matrix_close(actual, expected, rtol=0.02, atol=1e-4, label=""):
    for i in range(N):
        for j in range(N):
            a = float(actual[i][j])
...
async def test_identity(dut):
    ...
    X = (np.arange(N * N, dtype=np.float32) % 4 + 1).reshape(N, N)
...
async def test_all_ones(dut):
    ...
    result = await run_matmul(dut, mem, W, X)
    assert_matrix_close(result, bf16_ref(W, X), label="[all_ones] ")
...
async def test_zero_input(dut):
    ...
    result = await run_matmul(dut, mem, W, X)
    assert_matrix_close(result, np.zeros((N, N), dtype=np.float32), label="[zero_x] ")
...
async def test_random_matrices(dut):
    """Random small-integer matrices (range -3 to 3), 5 cases."""
    clock = Clock(dut.clk, 10, unit="ns")
    cocotb.start_soon(clock.start())
...
async def test_back_to_back(dut):
    ...
    X1 = (np.arange(N * N, dtype=np.float32) % 4 + 1).reshape(N, N)
...
async def test_diagonal_weight(dut):
    """Diagonal weight: W = diag(1,2,...,N mod 4 + 1) — small exact values."""
    clock = Clock(dut.clk, 10, unit="ns")
    cocotb.start_soon(clock.start())
    await reset_dut(dut)
    mem = {}
...
async def test_single_element(dut):
    """Only W[0][0]=1, rest zero. OUT[r][0] = X[r][0]."""
    clock = Clock(dut.clk, 10, unit="ns")
    cocotb.start_soon(clock.start())
    await reset_dut(dut)
    mem = {}
    ...
    result = await run_matmul(dut, mem, W, X)
    assert_matrix_close(result, bf16_ref(W, X), label="[single_elem] ")
...
async def test_permutation_matrix(dut):
    ...
    result = await run_matmul(dut, mem, W, X)
    assert_matrix_close(result, bf16_ref(W, X), label="[perm] ")
...
async def test_alternating_signs(dut):
    ...
    result = await run_matmul(dut, mem, W, X)
    assert_matrix_close(result, bf16_ref(W, X), label="[alt_sign] ")
...
async def test_large_values(dut):
    ...
    result = await run_matmul(dut, mem, W, X)
    assert_matrix_close(result, bf16_ref(W, X), label="[large_val] ")
    cocotb.log.info("PASS: large values")


@cocotb.test()
async def test_single_row_input(dut):
    """Only row 0 of X is nonzero."""
    clock = Clock(dut.clk, 10, unit="ns")
    cocotb.start_soon(clock.start())
    await reset_dut(dut)
    mem = {}
    ...
    result = await run_matmul(dut, mem, W, X)
    assert_matrix_close(result, bf16_ref(W, X), label="[single_row] ")
...
async def test_triple_back_to_back(dut):
    ...
    X_small = (np.arange(N * N, dtype=np.float32) % 4 + 1).reshape(N, N)
    configs = [
        (np.eye(N, dtype=np.float32),           X_small),
        (2.0 * np.eye(N, dtype=np.float32),     np.ones((N, N), dtype=np.float32) * 3.0),
        (-1.0 * np.eye(N, dtype=np.float32),    X_small),
    ]

    for idx, (W, X) in enumerate(configs):
        result = await run_matmul(dut, mem, W, X)
        assert_matrix_close(result, bf16_ref(W, X), label=f"[triple_b2b_{idx}] ")
